(function ($) {
        ...
        render: function (data, type, row, meta) {
          ...
          if (data[1].IsDirectory) {
            ...
            if (data[1].Ext == ".mp4" || data[1].Ext == ".mpg" || data[1].Ext == ".mkv") {
              ...
              let subtitles = "";
              console.log("data[1].Subtitles : " + data[1].Subtitles);
              if (data[1].Subtitles != "")
                subtitles =
                  "<track src='/thumb" +
                  data[1].FileUrl.substring(0, data[1].FileUrl.lastIndexOf("/") + 1) +
                  data[1].Subtitles +
  ...
  async function getConsole() {
    $.get("/getconsole").then(function (data) {
      if (data && data !== "") {
        ...
        if ($("#isGoLastLine").is(":checked")) scrollLogToBottom();
      }
    });
  }
  ...
  $('a[data-toggle="tab"]').on("shown.bs.tab", function (e) {
    ...
    if (e.target.toString().endsWith("#gettutorial")) {
      ...
      timer = setInterval(getConsole, 10000);
    }
